...
class Dechet {
    ...
    /**
     * Recherche multi-champs : type_aliment, raison, notes, categorie nom
     */
    ...
    public function getStatsByRaison(): array {
        return $this->pdo->query(
            "SELECT raison, COUNT(*) AS nb, SUM(quantite) AS total_kg
             FROM dechets GROUP BY raison ORDER BY total_kg DESC"
    ...
    public function getStatsByCategorie(): array {
        return $this->pdo->query(
            "SELECT COALESCE(c.nom, 'Sans catégorie') AS categorie,
                    COALESCE(c.couleur, '#81c784') AS couleur,
                    COUNT(*) AS nb, SUM(d.quantite) AS total_kg
             FROM dechets d
             LEFT JOIN categories c ON c.id = d.categorie_id
             GROUP BY c.id, c.nom, c.couleur ORDER BY total_kg DESC"
        )->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getStatsByMonth(): array {
        return $this->pdo->query(
            "SELECT DATE_FORMAT(date_dechet, '%Y-%m') AS mois,
                    COUNT(*) AS nb, SUM(quantite) AS total_kg
             FROM dechets GROUP BY mois ORDER BY mois ASC LIMIT 12"
        )->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getStatsByType(): array {
        return $this->pdo->query(
            "SELECT type_aliment, COUNT(*) AS nb, SUM(quantite) AS total_kg
             FROM dechets GROUP BY type_aliment ORDER BY total_kg DESC LIMIT 10"
        )->fetchAll(PDO::FETCH_ASSOC);
    }
}
