...
class Collecte {
    ...
    public function findAll(int $limit = 500, int $offset = 0): array {
        ...
        $st->execute();
        return array_map(fn($r) => (new self())->hydrate($r), $st->fetchAll(PDO::FETCH_ASSOC));
    }

    /**
     * Recherche multi-champs : titre, lieu, description, statut
     */
    public function search(string $q, int $limit = 500): array {
        $like = '%' . $q . '%';
        $sql  = "SELECT * FROM collectes
                 WHERE titre       LIKE :q1
                    OR lieu        LIKE :q2
                 ...
                 LIMIT :lim";
        $st = $this->pdo->prepare($sql);
        $st->bindValue(':q1', $like); $st->bindValue(':q2', $like);
        $st->bindValue(':q3', $like); $st->bindValue(':q4', $like);
        $st->bindValue(':lim', $limit, PDO::PARAM_INT);
        $st->execute();
        return array_map(fn($r) => (new self())->hydrate($r), $st->fetchAll(PDO::FETCH_ASSOC));
    }
    ...
    /**
     * Jointure 3 entités : collecte_dechets → dechets → categories
     * Retourne tous les déchets liés à cette collecte avec leur catégorie.
     */
    public function getDechetsAvecCategories(): array {
        if (!$this->id) return [];
        $sql = "SELECT
                    d.id             AS dechet_id,
                    d.type_aliment,
                ...
                FROM  collecte_dechets  cd
                JOIN  dechets           d   ON d.id   = cd.dechet_id
                LEFT JOIN categories    cat ON cat.id = d.categorie_id
                WHERE cd.collecte_id = :cid
                ORDER BY cat.nom ASC, d.type_aliment ASC";
        $st = $this->pdo->prepare($sql);
        $st->execute([':cid' => $this->id]);
        return $st->fetchAll(PDO::FETCH_ASSOC);
    }

    /**
     * Jointure 3 entités globale : résumé par collecte + catégories de déchets.
     */
    public static function getRecapCategories(PDO $pdo): array {
        $sql = "SELECT
                    ...
                    cat.icone          AS categorie_icone,
                    cat.couleur        AS categorie_couleur,
                    COUNT(d.id)        AS nb_dechets,
                    SUM(d.quantite)    AS total_kg
                FROM  collectes         co
                JOIN  collecte_dechets  cd  ON cd.collecte_id = co.id
                JOIN  dechets           d   ON d.id           = cd.dechet_id
                LEFT JOIN categories    cat ON cat.id          = d.categorie_id
                GROUP BY co.id, cat.id
                ORDER BY co.date_collecte DESC, total_kg DESC";
        return $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
    }

    /* ══════════════════════════════
       GESTION PIVOT collecte_dechets
    ══════════════════════════════ */
