...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            ...
            if (isset($_GET['recap_categories'])) {
                // Jointure 3 entités : collectes → collecte_dechets → dechets → categories
                $db = Database::getConnection();
                respond(200,['success'=>true,'data'=>Collecte::getRecapCategories($db)]);
            }
            if (isset($_GET['id'])) {
                $col = $m->findById((int)$_GET['id']);
                if (!$col) respond(404,['success'=>false,'message'=>'Introuvable']);
                $data = $col->toArray();
                // Enrichit avec la jointure 3 entités si demandé
                if (isset($_GET['with_dechets'])) {
                    $data['dechets'] = $col->getDechetsAvecCategories();
                }
