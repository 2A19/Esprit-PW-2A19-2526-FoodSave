// modifier-avis.js - Validation du formulaire de modification d'avis (Admin)

document.addEventListener('DOMContentLoaded', function() {
    
    console.log("JS chargé - modifier-avis.js (Admin)");
    
    const formulaire = document.getElementById('editAvisForm');
    
    if (!formulaire) {
        console.error("Formulaire avec id='editAvisForm' non trouvé !");
        return;
    }
    ...
    if (contenuTextarea) {
        contenuTextarea.addEventListener('input', function() {
            ...
            if (this.value.trim() === '') {
                afficherErreur(this, "Le contenu de l'avis est obligatoire");
            } else if (longueurSansEspaces < 5) {
                afficherErreur(this, "L'avis doit contenir au moins 5 caractères (actuellement: " + longueurSansEspaces + ")");
            } else if (longueur > 500) {
                afficherErreur(this, "L'avis ne doit pas dépasser 500 caractères (actuellement: " + longueur + ")");
            } else {
                effacerErreur(this);
            }
        });
    ...
    formulaire.addEventListener('submit', function(event) {
        ...
        if (!noteValue) {
            ...
            if (ratingContainer) {
                afficherErreur(ratingContainer, "Veuillez sélectionner une note");
            }
        } else if (noteValue < 1 || noteValue > 5) {
            messagesErreur.push("❌ La note doit être comprise entre 1 et 5");
            estValide = false;
        } else {
            const ratingContainer = document.querySelector('.rating');
            if (ratingContainer) {
                effacerErreur(ratingContainer);
            }
        }
        
        // Validation du contenu
        if (!contenuValue || contenuValue.trim() === '') {
            messagesErreur.push("❌ Le contenu de l'avis est obligatoire");
            estValide = false;
            if (contenuTextarea) {
                afficherErreur(contenuTextarea, "Le contenu de l'avis est obligatoire");
            }
        } 
        else if (longueurSansEspaces < 5) {
            messagesErreur.push("❌ L'avis doit contenir au moins 5 caractères (actuellement: " + longueurSansEspaces + ")");
            estValide = false;
            if (contenuTextarea) {
                afficherErreur(contenuTextarea, "L'avis doit contenir au moins 5 caractères");
            }
        }
        else if (longueurTotale > 500) {
            messagesErreur.push("❌ L'avis ne doit pas dépasser 500 caractères (actuellement: " + longueurTotale + ")");
            estValide = false;
            if (contenuTextarea) {
                afficherErreur(contenuTextarea, "L'avis ne doit pas dépasser 500 caractères");
            }
        }
        else {
            if (contenuTextarea) {
